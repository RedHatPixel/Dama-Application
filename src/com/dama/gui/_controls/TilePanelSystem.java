...
import com.dama.gui.gamePanel.BoardPanel;
import com.dama.gui.gamePanel.DragGlassPane;
import com.dama.gui.gamePanel.TablePanel;
import com.dama.gui.gamePanel.TilePanel;
import java.awt.*;
...
public final class TilePanelSystem implements MouseListener {
    ...
    private void createMove() {
        ...
        if (transition.getMoveStatus().isDone()) {
            TableManager.setNewBoard(getTable(), getTable().getGamePlay(), transition.getTransitionBoard());
            TableManager.setPlayersTimerMoveInterval(getTable());
            if (move.getType().canAttack()) {
                SoundManager.Sounds.MOVE_CAPTURE_SOUND.play();
            } else {
    ...
    private void validateMove() {
        ...
        if (getGameBoard().getCurrentPlayer().hasNoAvailablePieces()|| getGameBoard().getCurrentPlayer().hasNoLegalMoves()) {
            TableManager.setWinner(getTable(), true);
        }
        // Check if game was not progressing
        else if (GamePlay.calculate50LatestMove(getTable().getGamePlay().getBoardHistory()).isEmpty()) {
            TableManager.setStalemate(getTable());
        }
        // Flip board game play
        else if (getTable().getGameSetting().canChangeTurn()) {
            if (!getGameBoard().getLatestMove().getType().canAttackAgain()) {
